...
function Settings({ overview, choose, refresh, notify }: { overview: Overview; choose: () => void; refresh: () => Promise<void>; notify: (text: string, error?: boolean) => void }) {
  ...
  return <div className="subpage"><header><div><p className="eyebrow">偏好设置</p><h1>应用设置</h1><p className="subtitle">保持简单，也保留必要的控制。</p></div></header>
    ...
    <section className="settings-card"><div className="setting-row"><div className="setting-icon"><Icon name="refresh"/></div><div className="setting-copy"><h3>应用更新</h3><p>{readyVersion ? `ChatNest v${readyVersion} 已准备好` : `当前版本 v${overview.version}`}</p><small>{readyVersion ? '重启后将自动完成升级' : '检查并安装 ChatNest 最新版本'}</small></div><button className="secondary" disabled={updateBusy} onClick={update}>{updateBusy ? '检查并下载中…' : readyVersion ? '重启并升级' : '检查更新'}</button></div></section>
